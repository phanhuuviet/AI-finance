<script>
  ...
  import ChatHistory from "./ChatHistorySectioned.svelte";
  ...
  $: chatSection = $workspaceStore.activeSection;
  $: activeSessionId =
    $workspaceStore.currentSessionBySection?.[chatSection] ?? null;
  ...
  async function handleNewChat() {
    ...
    try {
      ...
      // New chat belongs to the Chat section by default
      workspaceStore.setActiveSection("chat");
      workspaceStore.setCurrentSessionForSection("chat", created?._id);
  ...
  function setSection(section) {
    workspaceStore.setActiveSection(section);
  }
</script>
...
<div class="flex h-screen bg-gray-50 overflow-hidden">
  ...
  <main class="flex-1 flex flex-col h-full overflow-hidden relative">
    {#if activeTab === "home"}
      ...
      >
        <h2 class="text-xl font-semibold text-gray-900">Chat</h2>
        ...
        <div class="flex items-center gap-3">
          ...
          >
            <button
              class={`px-3 py-1.5 rounded-lg text-sm ${chatSection === "documents" ? "bg-white shadow-sm border border-gray-200 text-gray-900" : "text-gray-600 hover:text-gray-900"}`}
            ...
            <button
              class={`px-3 py-1.5 rounded-lg text-sm ${chatSection === "chat" ? "bg-white shadow-sm border border-gray-200 text-gray-900" : "text-gray-600 hover:text-gray-900"}`}
            ...
            <button
              class={`px-3 py-1.5 rounded-lg text-sm ${chatSection === "studio" ? "bg-white shadow-sm border border-gray-200 text-gray-900" : "text-gray-600 hover:text-gray-900"}`}
      ...
      <div class="flex-1 overflow-auto p-6">
        <div class="flex h-full gap-6">
          <div class="w-1/3 min-w-[300px] h-full">
            <ChatHistory section={chatSection} />
          </div>

          <div class="flex-1 h-full">
            {#if chatSection === "documents"}
              <!-- Documents tab has its own history selection too; right panel is documents UI -->
              <DocumentUpload />
            {:else if chatSection === "studio"}
              <StudioPanel />
            {:else}
