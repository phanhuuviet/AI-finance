<script>
  ...
  let activeTab = "chat"; // 'chat', 'documents', 'analytics', 'settings'
  let isNewChatModalOpen = false;
  let newChatTitle = "";
  let selectedDocs = [];

  // This would ideally be fetched from a document store
  let availableDocs = [];
  ...
  async function handleNewChat() {
    ...
    try {
      await chatStore.createSession(newChatTitle, selectedDocs);
...
<div class="flex h-screen bg-gray-50 overflow-hidden">
  ...
  <aside class="w-64 bg-white border-r border-gray-200 flex flex-col">
    ...
    <nav class="flex-1 p-4 space-y-2 overflow-y-auto">
      <button
        class={`w-full flex items-center gap-3 px-4 py-3 rounded-lg text-left transition-colors ${activeTab === "chat" ? "bg-blue-50 text-blue-700 font-medium" : "text-gray-600 hover:bg-gray-100"}`}
        on:click={() => (activeTab = "chat")}
      >
        Chat
      </button>
      <button
        class={`w-full flex items-center gap-3 px-4 py-3 rounded-lg text-left transition-colors ${activeTab === "documents" ? "bg-blue-50 text-blue-700 font-medium" : "text-gray-600 hover:bg-gray-100"}`}
  ...
  <main class="flex-1 flex flex-col h-full overflow-hidden relative">
    <!-- Top Header for Chat Tab -->
    {#if activeTab === "chat"}
      <header
        class="bg-white border-b border-gray-200 px-6 py-4 flex justify-between items-center z-10"
      >
