<script lang="ts">
  ...
  function canRetrySub(job: VideoSubJob): boolean {
    if (job.status === RENDER_JOB_STATUS.FAILED) return true;
    if (job.status === RENDER_JOB_STATUS.PROCESSING) {
      return isProcessingTimeoutExceeded(job.claimed_at);
    }
    return false;
  }
    ...
    <div class="space-y-3">
      ...
      {:else}
        {#each $subJobs as job, i (job.id)}
          <div class="w-full text-left animate-fade-in-up" style={`animation-delay:${i * 70}ms`}>
                ...
                <div class="relative w-full aspect-video bg-[#0b0b0f]">
                  ...
                  {:else}
                    <div class="absolute inset-0 px-4 py-6 flex flex-col items-center justify-center gap-2 text-center
                      ...
                      {job.status === RENDER_JOB_STATUS.PROCESSING ? 'bg-amber-50 text-amber-700' : ''}
              ...
              <div class="flex-1 min-w-0 p-4 flex flex-col gap-3">
                <div class="flex items-center justify-between gap-2">
                  ...
                  <div class="relative flex items-center gap-2 flex-shrink-0">
                    ...
                    {#if canRetrySub(job) && openMenuSubId === job.id}
